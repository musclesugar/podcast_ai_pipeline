...
class AudioProcessor:
    ...
    def process_script_to_audio(self, script: str, speaker_voices: Dict[str, str], 
                               speaker_engines: Dict[str, str], output_path: Path, 
                               target_minutes: int = None, speed_scale: float = 1.0) -> None:
        ...
        dialogue_lines = self.script_parser.validate_and_preview_script(script, expected_speakers)
        ...
        audio_clips = self._generate_audio_clips(dialogue_lines, speaker_voices, speaker_engines, speed_scale)
    ...
    def _predownload_piper_voices(self, speaker_voices: Dict[str, str], speaker_engines: Dict[str, str]) -> None:
        """Pre-download all required Piper voices to avoid repeated messages."""
    ...
    def _generate_audio_clips(self, dialogue_lines: List[Tuple[str, str]], 
                             speaker_voices: Dict[str, str], speaker_engines: Dict[str, str], 
                             speed_scale: float) -> List[AudioSegment]:
        ...
        try:
            ...
            if speed_scale != 1.0:
                ...
                print(f"🎛️  Speed: {speed_scale}x ({int((speed_scale-1)*100):+d}% {speed_desc})")
            
            # Use a single progress bar that stays on one line
            with tqdm(total=len(dialogue_lines), desc="TTS Synthesis", 
    ...
    def _combine_audio_clips(self, clips: List[AudioSegment]) -> AudioSegment:
        ...
        with tqdm(total=len(clips), desc="Audio Assembly",
    ...
    def _export_audio(self, audio: AudioSegment, output_path: Path, target_minutes: int = None) -> None:
    ...
    def _show_duration_feedback(self, actual_minutes: float, target_minutes: int) -> None:
        """Show feedback about the generated audio duration."""
        if actual_minutes < target_minutes * 0.8:  # If significantly shorter than target
            ...
            print(f"📈 Audio is longer than target ({target_minutes} min) - great for detailed content!")
        else:
