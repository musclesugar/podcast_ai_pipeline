...
from config.settings import PIPER_DATA_DIR, PIPER_BASE_URL, DOWNLOAD_CHUNK_SIZE
...
class PiperTTS(TTSEngine):
    ...
    def synthesize(self, text: str, voice: str, output_path: Path, speed_scale: float = 1.0) -> None:
        ...
        try:
            ...
            cmd = ["piper", "--model", model_path, "--output_file", str(output_path), "--text", cleaned_text]
    ...
    def _download_voice(self, voice_name: str) -> str:
        ...
        print(f"   👤 {voice_info['gender'].title()} voice, {voice_info['quality']} quality")
        ...
        try:
            ...
            for voice in ["en_US-lessac-medium", "en_US-ryan-medium", "en_US-amy-medium"]:
                if voice != voice_name:
                    print(f"      --speakers '{{\"HOST\":\"{voice}\", \"GUEST\":\"{voice}\"}}'")
            raise
